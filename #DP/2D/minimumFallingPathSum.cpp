...
int fun(int m, int n, vector<vector<int>>& dp,vector<vector<int>> &mat) {
	...
	return dp[m][n] = mat[m][n] + min( u,min(ld,rd) ) ;
}


    int minFallingPathSum(vector<vector<int>>& mat) {

	// dp ( memoization   n ( for all elements of last row ) *(m*n*3) , m*n + stack(n-1) )
    ...
    //             dp[i][j] = mat[i][j] + min( u,min(ld,rd) );
    //         }
    //     }            
    // }
    // int ans=INT_MAX;
